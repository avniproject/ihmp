...
create or replace view ihmp_registration_view as (SELECT
  individual.uuid "Ind.uuid",
  individual.first_name "Ind.first_name",
  individual.last_name "Ind.last_name",
  ...
  (extract(years from age(individual.date_of_birth))*365 + extract(months from age(individual.date_of_birth))*30 + extract(days from age(individual.date_of_birth)))/365 "Ind.age",
  individual.date_of_birth_verified "Ind.date_of_birth_verified",
  individual.registration_date "Ind.registration_date",
  individual.facility_id  "Ind.facility_id",
  individual.is_voided "Ind.is_voided",
...
drop view if exists ihmp_needs_assessment_view;
  ...
  SELECT individual.uuid                                                                                        "Ind.uuid",
         ...
         multi_select_coded(programEncounter.observations->
           ...
           "Enc.Symptoms of RTI", single_select_coded(programEncounter.observations->>
                                                      '9947d31c-e70a-4cee-8a7e-63293ce3af9b')::TEXT as
           "Enc.Whether RTI cured", single_select_coded(programEncounter.observations->>
                                                        '3674585b-0ba3-4aeb-8e5a-901ab09ebf37')::TEXT as
           "Enc.Whether treatment for RTI symptom(s) taken", programEncounter.cancel_date_time
           "EncCancel.cancel_date_time", single_select_coded(programEncounter.cancel_observations->>
                                                             '739f9a56-c02c-4f81-927b-69842d78c1e8')::TEXT as
           "EncCancel.Visit cancel reason", programEncounter.cancel_observations->>
                                            '6539ec90-14b9-4e5f-9305-4ca05f89db50'::TEXT as
           "EncCancel.Follow up visit date for RTI", programEncounter.cancel_observations->>
                                                     'de3e8247-18e7-44aa-934e-6bd262251bd8'::TEXT as
           "EncCancel.Follow up visit date for FP", programEncounter.cancel_observations->>
                                                    'f6594bb7-db18-49cf-9987-eacbceb8f479'::TEXT as
           "EncCancel.Next ASHA ANC visit date", programEncounter.cancel_observations->>
                                                 'b09e425a-8f06-4cb4-9b71-5e26613ea22c'::TEXT as
           "EncCancel.Next needs assessment date", programEncounter.cancel_observations->>
                                                   'f23251e2-68c6-447b-84c2-285d61e95f0f'::TEXT as
           "EncCancel.Other reason for cancelling", programEncounter.cancel_observations->>
  ...
  WHERE c2.name not ilike '%master%'
    ...
    AND programEncounter.encounter_date_time IS NOT NULL AND programEnrolment.enrolment_date_time IS NOT NULL
);
